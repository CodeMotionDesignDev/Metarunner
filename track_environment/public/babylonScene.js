...
var createScene = function () {
	...
	s.onload = function () {
		...
		for (var l = 0; l < mapSubZ; l++) {
			for (var w = 0; w < mapSubX; w++) {
				...
				if (Math.random() > 0.85) {
					...
					// if (index % 3 === 0) {
					// 	SPmapData[0].push(xp, yp, zp, 0, ry, 0, sx, sy, sz);
					// } else {
					// 	SPmapData[1].push(xp, yp, zp, 0, ry, 0, sx, sy, sz);
					// }
				}
			}
		}
		...
		Promise.all([
			BABYLON.SceneLoader.ImportMeshAsync(null, "terrain_models/track/", "track.gltf", scene).then(function (result) {
				var groundMeshes = []
				var ground_plane = scene.getTransformNodeByName("TRACK1 GroundPlane");
				var inner_bank = scene.getTransformNodeByName("TRACK1 innerBank");
				var outer_bank = scene.getTransformNodeByName("TRACK1 outerBank");
				groundMeshes = ground_plane.getChildMeshes();
				innerMeshes = inner_bank.getChildMeshes();
				outerMeshes = outer_bank.getChildMeshes();
				var trackMeshes = [groundMeshes[0], innerMeshes[0], outerMeshes[0]]
				// ground_model = BABYLON.Mesh.MergeMeshes(groundMeshes)
				// console.log(groundMeshes);
				// console.log(innerMeshes);
				// console.log(outerMeshes);
				// console.log(trackMeshes);
				// console.log(ground_plane);
				// console.log(inner_bank);
				// console.log(outer_bank);
			}),
			BABYLON.SceneLoader.ImportMeshAsync(null, "terrain_models/", "Bush1.glb", scene).then(function (result) {
				var modelMeshes = []
				var rootNode = scene.getTransformNodeByName("Icosphere");
				modelMeshes = rootNode.getChildMeshes();
				model1 = BABYLON.Mesh.MergeMeshes(modelMeshes)
			}),
			BABYLON.SceneLoader.ImportMeshAsync(null, "terrain_models/", "Tree3.glb", scene).then(function (result) {
				var modelMeshes = []
				var rootNode = scene.getTransformNodeByName("GEO");
				modelMeshes = rootNode.getChildMeshes();
				model2 = BABYLON.Mesh.MergeMeshes(modelMeshes)
			}),
			BABYLON.SceneLoader.ImportMeshAsync(null, "terrain_models/", "Tree1.glb", scene).then(function (result) {
				var modelMeshes = []
				var rootNode = scene.getTransformNodeByName("Icosphere.002");
				modelMeshes = rootNode.getChildMeshes();
				model3 = BABYLON.Mesh.MergeMeshes(modelMeshes)
			})
		]).then(() => {
			var typ1 = SPS.addShape(model1, 500);
			var typ2 = SPS.addShape(model2, 500);
			var typ3 = SPS.addShape(model3, 500);
